  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className='flex items-center'>
        <img src={theme === 'dark' ? assets.logo_full : assets.logo_full_dark } alt='' className='w-full max-w-22' /> <p className='text-2xl dark:text-white'><i>Axtro AI</i></p>
      </div>
      ...
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
        ...
        <input onChange={(e)=>setSearch(e.target.value)} value={search} type="text" placeholder='Buscar conversaciones' className='text-xs placeholder:text-gray-400 outline-none' />
      </div>

      {/* Resent Chats */}
      {chats.length > 0 && <p className='mt-4 text-sm dark:text-white'>Chats recientes</p>}
      <div className='flex-1 min-h-0 overflow-y-auto custom-scrollbar mt-3 text-sm space-y-3 pr-2'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLocaleLowerCase()) : chat.name.toLowerCase().includes(search.toLocaleLowerCase())).map((chat) => (
            <div onClick={()=>{navigate('/'); setSelectedChat(chat); setIsMenuOpen(false)}} key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border flex justify-between group dark:text-white'>
              <div>
      ...
      {/* Dark Mode Toggle */}
      <div className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounder-md'>
